...
// All third party code is listed here:	 Joey De Vries, Learn OpenGL, Getting started, Textures. Available here:  https://learnopengl.com/Getting-started/Textures 
//										
#include "model.h"
#include <iostream>
using namespace std;
...
#define CHECKTEXTURE "Checkboard2.jpg"
...
void display_object(Shader shader_obj, Model model_object, int matrix_location, mat4 model) {
	...
	//Texture locations
	GLint  texLoc = glGetUniformLocation(shader_obj.shaderProgramID, "normalMap");
	glUniform1i(texLoc, 0);

	GLint texLoc2 = glGetUniformLocation(shader_obj.shaderProgramID, "textureMap");
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, model_object.normalmap);
...
void display_skybox(Shader shader_skybox, mat4 model, glm::mat4 view, glm::mat4 persp_proj) {
...
void display_light(Shader shader_obj, Model model_object, mat4 model, mat4 light_matrix, glm::mat4 view, glm::mat4 persp_proj) {

	glActiveTexture(GL_TEXTURE0);
	glUseProgram(shader_obj.shaderProgramID);
	glm::mat4 model_light = glm::mat4(1.0f);
	model_light = glm::translate(model_light, glm::vec3(x, y, z));
	int matrix_locationlight = glGetUniformLocation(shader_obj.shaderProgramID, "model");
	glUniformMatrix4fv(glGetUniformLocation(shader_obj.shaderProgramID, "model"), 1, GL_FALSE, &model_light[0][0]);
	glUniformMatrix4fv(glGetUniformLocation(shader_obj.shaderProgramID, "view"), 1, GL_FALSE, &view[0][0]);
	glUniformMatrix4fv(glGetUniformLocation(shader_obj.shaderProgramID, "proj"), 1, GL_FALSE, &persp_proj[0][0]);
	GLfloat light_vec3[3] = { 1.0, 1.0, 1.0 }; //colour
	glUniform3fv(glGetUniformLocation(shader_obj.shaderProgramID, "lightcolour"), 1, &light_vec3[0]);
	glBindVertexArray(light1.modelvao);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, light1.texture);
	glUniformMatrix4fv(matrix_locationlight, 1, GL_FALSE, light_matrix.m);
...
void display() {
	...
	display_light(shader_light, light1, model, light_matrix, view, persp_proj);
	...
	if (shaderSpecifier == 7) { //phong, no mip maps
		objectColor = greyish;
		setupgeneric(shader_object2.shaderProgramID, model_shader, view, persp_proj, 90.0, matrix_location);
		display_object(shader_object2, monkeymodel, matrix_location, model);// MONKEY HEAD
		model = translate(model, vec3(-8.0f, 0.0f, 0.0f));
		objectColor = red;
		display_object(shader_object2, teamugmodel, matrix_location, model);// MODEL TEAPOT
		model = translate(model, vec3(0.0f, 0.0f, -8.0f));
		objectColor = orangeish;
		display_object(shader_object2, teapotmodel, matrix_location, model); // MODEL TEAMUG
		planeMatrix = translate(planeMatrix, vec3(0.0f, -3.0f, 0.0f));
		display_object(shader_object2, planemodel, matrix_location, planeMatrix); // MODEL plane
	}
	if (shaderSpecifier == 8) { //phong, mip maps
		objectColor = greyish;
		setupgeneric(shader_object2.shaderProgramID, model_shader, view, persp_proj, 90.0, matrix_location);
		display_object(shader_object2, monkeymodel2, matrix_location, model); // MONKEY HEAD
		model = translate(model, vec3(-8.0f, 0.0f, 0.0f));
		objectColor = red;
		display_object(shader_object2, teamugmodel2, matrix_location, model); // MODEL TEAPOT
		model = translate(model, vec3(0.0f, 0.0f, -8.0f));
		objectColor = orangeish;
		display_object(shader_object2, teapotmodel2, matrix_location, model); // MODEL TEAMUG
		planeMatrix = translate(planeMatrix, vec3(0.0f, -3.0f, 0.0f));
		display_object(shader_object2, planemodel2, matrix_location, planeMatrix); // MODEL plane
	}
	if (shaderSpecifier == 9) { //phong, normal maps, no mip maps
		objectColor = greyish;
		setupgeneric(shader_object.shaderProgramID, model_shader, view, persp_proj, 90.0, matrix_location);
		display_object(shader_object, monkeymodel, matrix_location, model); // MONKEY HEAD
		model = translate(model, vec3(-8.0f, 0.0f, 0.0f));
		objectColor = red;
		display_object(shader_object, teamugmodel, matrix_location, model); // MODEL TEAPOT
		model = translate(model, vec3(0.0f, 0.0f, -8.0f));
		objectColor = orangeish;
		display_object(shader_object, teapotmodel, matrix_location, model);// MODEL TEAMUG
		planeMatrix = translate(planeMatrix, vec3(0.0f, -3.0f, 0.0f));
	}
	if (shaderSpecifier == 10) { //phong, normal maps, mip maps
		objectColor = greyish;
		setupgeneric(shader_object.shaderProgramID, model_shader, view, persp_proj, 90.0, matrix_location);
		display_object(shader_object, monkeymodel2, matrix_location, model); // MONKEY HEAD
		model = translate(model, vec3(-8.0f, 0.0f, 0.0f));
		objectColor = red;
		display_object(shader_object, teamugmodel2, matrix_location, model); // MODEL TEAPOT
		model = translate(model, vec3(0.0f, 0.0f, -8.0f));
		objectColor = orangeish;
		display_object(shader_object, teapotmodel2, matrix_location, model);// MODEL TEAMUG
		planeMatrix = translate(planeMatrix, vec3(0.0f, -3.0f, 0.0f));
	}
	...
	//Skybox
	display_skybox(shader_skybox, model, view, persp_proj);
...
void updateScene() {
	...
	animation_matrix2 = rotate_y_deg(animation_matrix2, -0.2f);
}
...
void init()
{
	...
	shader_light.shaderProgramID = shader_light.CompileShaders(0);

	shader_object.shaderProgramID = shader_object.CompileShaders(2); //phong + normal maps + textures
	shader_object2.shaderProgramID = shader_object2.CompileShaders(3); //phong + textures 
	...
	monkeymodel.setup(MONKEY, CHECKTEXTURE, STONENORMAL, shader_object.shaderProgramID, false);
	teapotmodel.setup(BOX, CHECKTEXTURE, STONENORMAL, shader_object.shaderProgramID, false);
	teamugmodel.setup(SPHERE, CHECKTEXTURE, STONENORMAL, shader_object.shaderProgramID, false);

	monkeymodel2.setup(MONKEY, CHECKTEXTURE, STONENORMAL, shader_object.shaderProgramID, true);
	teapotmodel2.setup(BOX, CHECKTEXTURE, STONENORMAL, shader_object.shaderProgramID, true);
	teamugmodel2.setup(SPHERE, CHECKTEXTURE, STONENORMAL, shader_object.shaderProgramID, true);

	monkeymodel3.setup(MONKEY, CHECKTEXTURE, STONENORMAL, shader_object.shaderProgramID, true);
	teapotmodel3.setup(BOX, CHECKTEXTURE, STONENORMAL, shader_object.shaderProgramID, true);
	teamugmodel3.setup(SPHERE, CHECKTEXTURE, STONENORMAL, shader_object.shaderProgramID, true);

	planemodel.setup(PLANE, CHECKTEXTURE, STONENORMAL, shader_object.shaderProgramID, false);
	planemodel2.setup(PLANE, CHECKTEXTURE, STONENORMAL, shader_object.shaderProgramID, true);

	//monkeymodel3.setup(MONKEY, CHECKTEXTURE, STONE2NORMAL, shader_object.shaderProgramID, true);
	//teapotmodel3.setup(BOX, CHECKTEXTURE, STONE2NORMAL, shader_object.shaderProgramID, true);
	//teamugmodel3.setup(SPHERE, CHECKTEXTURE, STONE2NORMAL, shader_object.shaderProgramID, true);

	light1.setup(SPHERE, CHECKTEXTURE, STONENORMAL, shader_light.shaderProgramID, true);
