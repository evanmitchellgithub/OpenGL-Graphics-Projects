...
void Model::GenerateObjectBufferMesh(const char* mesh_name, const char* texture_name, const char* normal_name, bool doMipMap) {
	/*----------------------------------------------------------------------------
	...
	unsigned char* data = stbi_load(texture_name, &width, &height, &nrChannels, 0); //using 0 as 5th causes access violations??
	if (data)
	{
		//for png files
		GLenum type = GL_RGBA;
...
void Model::setup(const char* mesh_name, const char* texture_name, const char* normal_name, GLuint shader, bool doMipMap) {
	shaderProgramID = shader;
	GenerateObjectBufferMesh(mesh_name, texture_name, normal_name, doMipMap);
}